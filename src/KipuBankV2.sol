...
contract KipuBankV2 is Ownable {
    ...
    constructor(uint256 _withdrawalLimit, uint256 _bankCap, address _ethUsdPriceFeed) Ownable(msg.sender) {
        WITHDRAWAL_LIMIT = _withdrawalLimit;
        BANK_CAP = _bankCap;

        // Add ETH as supported token
        supportedTokens[ETH_ADDRESS] =
            TokenInfo({isSupported: true, priceFeed: AggregatorV3Interface(_ethUsdPriceFeed), decimals: 18});
    ...
    function addSupportedToken(address token, address priceFeed) external onlyOwner {
        ...
        supportedTokens[token] = TokenInfo({isSupported: true, priceFeed: feed, decimals: decimals});
    ...
    {
        return (depositsCount, withdrawalsCount, totalDepositedUsd, BANK_CAP - totalDepositedUsd);
    }
    ...
    function _convertToUsd(address token, uint256 amount) internal view returns (uint256) {
        ...
        uint256 usdValue =
            (amount * uint256(price) * (10 ** USDC_DECIMALS)) / (10 ** tokenInfo.decimals * 10 ** priceDecimals);
